...
class StarKnifePGD(nn.Module):
    ...
    def attack_single_run(self, data, target, adv_elder):
        ...
        # return x_adv
        # Finetune
        x_final = x_adv.detach() + (torch.rand_like(data)*2-1)*self.eps/4
        ...
        for _ in range(steps_2):
            ...
            x_final = x_final.detach() + eta/4
            x_final = torch.min(torch.max(x_final, x_adv - self.eps/4), x_adv + self.eps/4)
