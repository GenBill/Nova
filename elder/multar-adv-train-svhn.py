...
def run(lr, epochs, batch_size, gamma=0.5):
    ...
    train_dataset = SVHN(os.environ['DATAROOT'], transform=train_transforms, train=True, max_n_per_class=500)
    ...
    test_dataset = SVHN(os.environ['DATAROOT'], transform=test_transforms, train=False, max_n_per_class=50)
    ...
    if torch.distributed.get_rank() == 0:
        ...
        torch.save(model.cpu(), './checkpoint/multar-plain-svhn-LRDLDE.pth')
        print('Save model.')
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter('./runs/svhn')
