...
from runner import FrostRunner_SR
...
def run(lr, epochs, batch_size):
    ...
    runner = FrostRunner_SR(epochs, model, train_loader, shadow_loader, test_loader, criterion, optimizer, scheduler, attacker, train_dataset.class_num, device)
    runner.eval_interval = 10
    runner.vertex_tar(writer)

    if torch.distributed.get_rank() == 0:
        torch.save(model.state_dict(), './checkpoint/vertex_tar_sr.pth')
        print('Save model.')
