...
def run(lr, epochs, batch_size):
    ...
    runner = FrostRunner(epochs, model, train_loader, test_loader, criterion, optimizer, scheduler, attacker, n_classes, device)
    runner.eval_interval = 10
    runner.vertex_tar(writer)

    if torch.distributed.get_rank() == 0:
        torch.save(model.state_dict(), './checkpoint/MedMSE/vertex_tar.pth')
        print('Save model.')
