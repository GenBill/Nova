...
class resnet18_small_rnn(nn.Module):
    def __init__(self, n_class, dimc=3, dimx=32, dimy=32, times=2):
    ...
    def forward(self, x):
        f = self.encoder(x)
        y = self.classifier(f)/(self.times+1)
        for _ in range(self.times):
            f = self.encoder(x + self.recurrent(f).view(-1,self.dimc,self.dimx,self.dimy))
            y += self.classifier(f)/(self.times+1)
