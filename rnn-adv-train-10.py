...
from model import resnet18_small_rnn
from runner import LinfRunner
...
def run(lr, epochs, batch_size):
    ...
    model = resnet18_small_rnn(train_dataset.class_num, 3, 32, 32, 2).to(device)
    ...
    runner = LinfRunner(epochs, model, train_loader, test_loader, criterion, optimizer, scheduler, attacker, device, train_dataset.class_num)
    runner.eval_interval = 10
    runner.train(writer)

    if torch.distributed.get_rank() == 0:
        torch.save(model.state_dict(), './checkpoint/adv_rnn.pth')
        print('Save model.')
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter('./runs/adv_rnn')
