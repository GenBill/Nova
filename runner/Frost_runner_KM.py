...
def false_target_attack(adversary, inputs, true_target, num_class, device):
    # Ensure target != true_target
    target = torch.randint(low=0, high=num_class-1, size=true_target.shape, device=device)
    target += (target >= true_target).int()
    
    adversary.targeted = True
    return adversary.perturb(inputs, target).detach(), target
...
def Plain_Mix_labels(inputs_1, inputs_2, false_labels, labels, num_class, device):
    rand_lambda = torch.rand((inputs_1.shape[0],1,1,1), device=device)
    data = rand_lambda*inputs_1 + (1-rand_lambda)*inputs_2
    targets = F.one_hot(labels, num_class).float()
    false_targets = F.one_hot(false_labels, num_class).float()
    rand_lambda.squeeze_(3).squeeze_(2)
    targets = targets - rand_lambda*false_targets/8
    return data, targets

class FrostRunner_KM():
    ...
    def vertex_tar_step(self, progress):
        ...
        for inputs, labels in self.train_loader:
            ...
            _model_freeze(self.model)
            adv_inputs, false_labels = false_target_attack(self.attacker, inputs, labels, self.num_class, self.device)
            _model_unfreeze(self.model)
            inputs, labels = Plain_Mix_labels(adv_inputs, inputs, false_labels, labels, self.num_class, self.device)
            # labels = F.one_hot(labels, self.num_class).float()
