...
class AdaRunner():
    def __init__(self, epochs, model, ada_net, train_loader, test_loader, criterion, optimizer, scheduler, attacker, device):
        self.device = device
        self.epochs = epochs
        self.eval_interval = 20

        self.model = model
        self.ada_net = ada_net
    ...
    def clean_eval(self, progress):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(self.test_loader):
                ...
                if batch_idx==0:
                    ...
                    long_tensor = torch.cat((long_tensor, output), dim=0)
        ...
        t_np = long_tensor.cpu().numpy()                    # convert to Numpy array
        df = pd.DataFrame(t_np)                             # convert to a dataframe
        df.to_csv("de_csv/clean_" + progress, index=False)  # save to file
    ...
    def adv_eval(self, progress):
        ...
        t_np = long_tensor.cpu().numpy()                    # convert to Numpy array
        df = pd.DataFrame(t_np)                             # convert to a dataframe
        df.to_csv("de_csv/adv_" + progress, index=False)    # save to file
